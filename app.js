...
function getImagePlacement(area, scale, offsetX, offsetY, expand = 1) {
  ...
  // 竖图：宽度=画布宽度×缩放，高度按比例计算（会超出画框）
  // 横图：高度=画布高度×缩放，宽度按比例计算（会超出画框）
  if (bgAspect < areaAspect) {
    baseWidth = area.width * (scale / 100);
    baseHeight = baseWidth / bgAspect;
  } else {
    baseHeight = area.height * (scale / 100);
    baseWidth = baseHeight * bgAspect;
  }
  const maxX = Math.max(0, (baseWidth - area.width) / 2);
  const maxY = Math.max(0, (baseHeight - area.height) / 2);
  const width = baseWidth * expand, height = baseHeight * expand;
  return {
    x: area.x + (area.width - baseWidth) / 2 + offsetX / 100 * maxX - (width - baseWidth) / 2,
    y: area.y + (area.height - baseHeight) / 2 + offsetY / 100 * maxY - (height - baseHeight) / 2,
    width, height
  };
}
function imageLayer(src, clipId, area, scale, offsetX, offsetY, { opacity = 1, filter = "", expand = 1, preserve = "none" } = {}) { const p = getImagePlacement(area, scale, offsetX, offsetY, expand); return `<image href="${src}" xlink:href="${src}" x="${p.x}" y="${p.y}" width="${p.width}" height="${p.height}" preserveAspectRatio="${preserve}" opacity="${opacity}" clip-path="url(#${clipId})"${filter ? ` filter="${filter}"` : ""}/>`; }
...
function scaleAround(asset, center, oldScale, newScale, oldX, oldY) {
  ...
  const areaAspect = area.width / area.height;
  let baseWidth, baseHeight;
  if (bgAspect > areaAspect) { baseHeight = area.height * (newScale / 100); baseWidth = baseHeight * bgAspect; }
  else { baseWidth = area.width * (newScale / 100); baseHeight = baseWidth / bgAspect; }
