...
const STORAGE_KEY = "achievement-unlock-editor-v5";
const LEGACY_STORAGE_KEYS = ["achievement-unlock-editor-v4", "achievement-unlock-editor-v3"];
...
const TYPOGRAPHY_PRESETS = {
  ...
  certificate: {
    ...
    title: { fontSize: 72, minFontSize: 52, fontWeight: 600, letterSpacing: 1, opacity: .96, color: "text", shadowColor: "#000000", shadowOpacity: .12, shadowBlur: 1.25, shadowOffsetY: 1, lineHeight: 1.12 },
    description: { fontSize: 30, fontWeight: 400, letterSpacing: 0, opacity: .72, color: "text", shadowColor: "#000000", shadowOpacity: .08, shadowBlur: 1, shadowOffsetY: 1, lineHeight: 52 },
...
function normalizeState() {
  ...
  if (!Number.isFinite(Number(state.subtitleOpacity))) state.subtitleOpacity = DEFAULTS.subtitleOpacity;
  if (!Number.isFinite(Number(state.titleOpacity))) state.titleOpacity = DEFAULTS.titleOpacity;
  if (!Number.isFinite(Number(state.descOpacity))) state.descOpacity = DEFAULTS.descOpacity;
...
function roleColor(token) { return token.color === "accent" ? state.accentColor : state.textColor; }
function currentTypography() {
  ...
  clone.subtitle.opacity = Number.isFinite(Number(state.subtitleOpacity)) ? Number(state.subtitleOpacity) : clone.subtitle.opacity;
...
function applyMaterial(key, source = "manual") { const preset = MATERIAL_PRESETS[key] || MATERIAL_PRESETS.standard; state.materialBase = preset.id; state.glassPreset = preset.id; state.glassDepth = preset.glassDepth; state.cardOpacity = preset.cardOpacity; state.glassDispersion = preset.glassDispersion; state.glassSaturation = preset.saturation; state.glassContrast = preset.contrast; state.glassTintColor = preset.tintColor; state.glassTintOpacity = preset.tintOpacity; state.textColor = preset.recommendedTextColor; state.primaryAccent = preset.recommendedAccentColor; state.accentColor = preset.recommendedAccentColor; state.selectedTheme = -1; const rec = preset.typographyRecommendation || {}; if (!state.typographyManuallyEdited && !state.customFontName) { state.titleFontFamily = rec.titleFontFamily || state.titleFontFamily; state.bodyFontFamily = rec.bodyFontFamily || state.bodyFontFamily; if (Number.isFinite(rec.titleLetterSpacing)) state.titleLetterSpacing = rec.titleLetterSpacing; } if (source === "manual") markSmartManual(); }
...
async function smartFitBackground() { if (smartBusy) return; const btn=$("smartFit"); smartBusy=true; btn.disabled=true; btn.dataset.label=btn.textContent; const steps=["正在分析 Logo 配色","正在分析背景材质","正在检查文字对比度","正在调整卡片布局"]; let step=0; const advance=()=>{btn.textContent=steps[Math.min(step++,steps.length-1)]; if($("smartSummary"))$("smartSummary").textContent=btn.textContent;}; try { beginInteraction(); advance(); if (state.logo && state.logoCropMode === "auto") await refreshLogoCropBounds(); const analysis = await analyzeBackground(); advance(); const chosen = chooseSmartMaterial(analysis); suppressSmartManual = true; applyMaterial(chosen.key, "smart"); state.glassSaturation = chosen.glass.saturation; state.glassContrast = chosen.glass.contrast; state.glassTintColor = chosen.glass.tintColor; state.glassTintOpacity = chosen.glass.tintOpacity; advance(); const material = { ...MATERIAL_PRESETS[chosen.key], saturation:state.glassSaturation, contrast:state.glassContrast, tintColor:state.glassTintColor, tintOpacity:state.glassTintOpacity }; const logoAnalysis = await analyzeLogoColors(chosen.key, analysis.text); const textPlan = autoTextAndOpacity(analysis, material, estimateDescriptionLineCount()); state.textColor = textPlan.textColor; state.titleOpacity = textPlan.titleOpacity; state.descOpacity = textPlan.descOpacity; state.titleShadowOpacity = textPlan.titleShadow; state.descShadowOpacity = textPlan.descShadow; state.primaryAccent = logoAnalysis.usable ? logoAnalysis.primaryAccent : safeAccentFromBackground(analysis, material); state.secondaryAccent = logoAnalysis.usable ? logoAnalysis.secondaryAccent : ""; state.accentColor = state.primaryAccent || material.recommendedAccentColor || "#9AA8BB"; state.subtitleOpacity = chosen.key === "obsidian" ? .88 : .82; advance(); applySmartLayoutForContent(); state.smartConfidence = chosen.confidence;
    ...
    showToast(reason);
    suppressSmartManual = false; syncUi(); scheduleRender(); endInteraction(); } catch (_) { suppressSmartManual = true; applyMaterial("standard", "smart"); state.glassSaturation=.96; state.glassContrast=1.03; state.glassTintColor="#FFFFFF"; state.glassTintOpacity=.12; state.primaryAccent=state.accentColor=MATERIAL_PRESETS.standard.recommendedAccentColor; state.secondaryAccent=""; state.subtitleOpacity=.82; state.titleOpacity=.96; state.descOpacity=.76; state.titleShadowOpacity=.12; state.descShadowOpacity=.10; state.smartConfidence="low"; applySmartLayoutForContent(); state.smartSummary="特征较弱，已应用标准安全方案。"; suppressSmartManual = false; syncUi(); scheduleRender(); endInteraction(); showToast("智能适配遇到异常，已应用标准安全方案。", "error", true); } finally { smartBusy=false; btn.disabled=false; btn.textContent=btn.dataset.label||"智能适配底图"; } }
