...
function getImagePlacement(area, scale, offsetX, offsetY, expand = 1) { const bgAspect = state.backgroundAspect || (area.width / area.height); const areaAspect = area.width / area.height; let baseWidth, baseHeight; if (bgAspect > areaAspect) { baseHeight = area.height * (scale / 100); baseWidth = baseHeight * bgAspect; } else { baseWidth = area.width * (scale / 100); baseHeight = baseWidth / bgAspect; } const maxX = Math.max(0, (baseWidth - area.width) / 2), maxY = Math.max(0, (baseHeight - area.height) / 2); const width = baseWidth * expand, height = baseHeight * expand; return { x: area.x + (area.width - baseWidth) / 2 + offsetX / 100 * maxX - (width - baseWidth) / 2, y: area.y + (area.height - baseHeight) / 2 + offsetY / 100 * maxY - (height - baseHeight) / 2, width, height }; }
