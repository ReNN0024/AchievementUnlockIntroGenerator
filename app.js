...
function contrastWithOpacity(hex, opacityPercent, bgLum) { const fg = hexToRgb(normalizeHex(hex) || "#000000"); const bg = bgLum > .5 ? { r: 255, g: 255, b: 255 } : { r: 0, g: 0, b: 0 }; const mixed = compositeRgb(fg, bg, clamp(opacityPercent, 0, 100) / 100); return contrastRatio(luminance(mixed), bgLum); }
...
function buildDefs(card, logoBox, material, typography) { const logoRadius = state.layoutPreset === "certificate" ? 44 : 60; const contrastIntercept = (1 - material.contrast) / 2; const edgeA = `M ${card.x + card.radius} ${card.y} H ${card.x + card.width - card.radius} Q ${card.x + card.width} ${card.y} ${card.x + card.width} ${card.y + card.radius} V ${card.y + card.height * .26}`; const edgeB = `M ${card.x + card.width - card.radius} ${card.y + card.height} H ${card.x + card.radius} Q ${card.x} ${card.y + card.height} ${card.x} ${card.y + card.height - card.radius} V ${card.y + card.height * .74}`; return `<defs>
    ...
    <linearGradient id="systemTitleRule" x1="0" y1="0" x2="1" y2="0"><stop offset="0" stop-color="${state.accentColor}" stop-opacity="${0.28 * clamp(state.dividerOpacity,0,100) / 100}"/><stop offset=".42" stop-color="${resolvedTextColor("title")}" stop-opacity="${0.12 * clamp(state.dividerOpacity,0,100) / 100}"/><stop offset="1" stop-color="${resolvedTextColor("title")}" stop-opacity="0"/></linearGradient>
...
function generateTextColorCandidates(logoAnalysis) { const base = { light:["#F4F1EC","#EFE4E5","#F3ECCF","#EEF3F2"], dark:["#514C52","#655A61","#4F5358","#5A514A","#4D5057"] }; const logo = logoAnalysis?.candidates?.[0]; if (logo) { base.light.push(rgbToHex(oklchToRgb({ L:.91, C:Math.min(.045, logo.C*.35), h:logo.h }))); base.dark.push(rgbToHex(oklchToRgb({ L:.43, C:Math.min(.045, logo.C*.35), h:logo.h }))); } return { light:[...new Set(base.light.map(normalizeHex).filter(Boolean))], dark:[...new Set(base.dark.map(normalizeHex).filter(Boolean))] }; }
function textCandidateContrast(hex, opacity, stats) { const median=contrastWithOpacity(hex,opacity,stats.median), bright=contrastWithOpacity(hex,opacity,stats.bright15), dark=contrastWithOpacity(hex,opacity,stats.dark15), avg=contrastWithOpacity(hex,opacity,stats.avg); const worst=Math.min(median,bright,dark); return { median, bright, dark, avg, worst, score: median*.35 + worst*.45 + avg*.20 }; }
function scoreTextCandidate(hex, opacity, stats, logoAnalysis, family) { const contrast=textCandidateContrast(hex,opacity,stats), rgb=hexToRgb(hex), hsl=rgbToHsl(rgb); const logoHue=logoAnalysis?.candidates?.[0]?.h; const harmony=Number.isFinite(logoHue) ? 1 - hueDistance(hsl.h, logoHue)/180 : .5; const softness=1-Math.min(1,hsl.s/.55); const materialFitness=family==="dark" ? (stats.median>.52?.9:.45) : (stats.median<.45?.9:.45); const hard=contrast.worst >= 2.85 && contrast.score >= 3.35; return { hex, opacity, family, contrast, source: family === "dark" ? "Logo 粉色衍生深灰紫" : "柔和浅色候选", score: hard ? contrast.score*.60 + harmony*.22 + softness*.10 + materialFitness*.08 : -100 + contrast.score }; }
function bestRoleCandidate(candidates, stats, logoAnalysis, role, keepOpacity) { const preferredFamily = stats.median > .56 || stats.p80 > .78 ? "dark" : (stats.median < .34 ? "light" : "mixed"); const op = keepOpacity ?? (role === "title" ? (preferredFamily === "dark" ? 94 : 90) : role === "body" ? 82 : 88); let pool = [...candidates.dark.map(hex=>scoreTextCandidate(hex,op,stats,logoAnalysis,"dark")), ...candidates.light.map(hex=>scoreTextCandidate(hex,op,stats,logoAnalysis,"light"))]; if (preferredFamily === "dark") pool.forEach(x=>{ if(x.family==="dark") x.score+=.45; }); if (preferredFamily === "light") pool.forEach(x=>{ if(x.family==="light") x.score+=.25; }); pool.sort((a,b)=>b.score-a.score); let best = pool[0]; const min = role === "body" ? 4.2 : 3.0; if (best.contrast.score < min) { const stronger = scoreTextCandidate(best.hex, Math.min(100, op + 6), stats, logoAnalysis, best.family); if (stronger.contrast.score > best.contrast.score) best = stronger; } return best; }
function computeAutoTextColorPlan(analysis, material, lineCount, logoAnalysis) { const stats=regionLumStats(analysis.title || analysis.text), candidates=generateTextColorCandidates(logoAnalysis), title=bestRoleCandidate(candidates,stats,logoAnalysis,"title"), bodyCandidates={ light:candidates.light, dark:["#746A70","#6B646A","#5E5A60",...candidates.dark] }, body=bestRoleCandidate(bodyCandidates,stats,logoAnalysis,"body"), accent=normalizeHex(state.accentColor)||MATERIAL_PRESETS[currentMaterialBase()].recommendedAccentColor; const subtitleHex = contrastWithOpacity(accent,88,stats.median) >= 3 ? accent : rgbToHex(oklchToRgb({ L:title.family === "dark" ? .50 : .76, C:.06, h:rgbToOklch(hexToRgb(accent)).h })); const subtitleContrast=textCandidateContrast(subtitleHex,88,stats); return { summaryFamily:title.family, title:{ color:title.hex, opacity:title.opacity, contrast:title.contrast.score, source:title.source, shadow:title.family === "dark" ? .02 : .08 }, body:{ color:body.hex, opacity:lineCount<=1?Math.max(80,body.opacity):Math.max(76,body.opacity-2), contrast:body.contrast.score, source:body.source === "Logo 粉色衍生深灰紫" ? "低饱和暖灰正文" : body.source, shadow:body.family === "dark" ? .02 : .07 }, subtitle:{ color:subtitleHex, opacity:88, contrast:subtitleContrast.score, source:"强调色对比修正" } }; }
async function applySmartTextColors({ resetOpacity = false } = {}) { const analysis = await analyzeBackground(); const material = currentMaterialToken(); const logoAnalysis = await analyzeLogoColors(currentMaterialBase(), analysis.text); const plan = computeAutoTextColorPlan(analysis, material, estimateDescriptionLineCount(), logoAnalysis); if (state.titleColorMode !== "manual") { state.titleColor = plan.title.color; state.titleColorMode = "auto"; state.titleColorSource = plan.title.source; } if (state.bodyTextColorMode !== "manual") { state.bodyTextColor = plan.body.color; state.bodyTextColorMode = "auto"; state.bodyTextColorSource = plan.body.source; } if (state.subtitleColorMode !== "manual") { state.subtitleColor = plan.subtitle.color; state.subtitleColorMode = "auto"; state.subtitleColorSource = plan.subtitle.source; } if (resetOpacity || !state.titleOpacityManuallyEdited) state.titleOpacity = plan.title.opacity; if (resetOpacity || !state.bodyTextOpacityManuallyEdited) state.bodyTextOpacity = plan.body.opacity; if (resetOpacity || !state.subtitleOpacityManuallyEdited) state.subtitleOpacity = plan.subtitle.opacity; state.titleContrast = plan.title.contrast; state.bodyTextContrast = plan.body.contrast; state.subtitleContrast = plan.subtitle.contrast; state.titleShadowOpacity = plan.title.shadow; state.descShadowOpacity = plan.body.shadow; syncLegacyTextColor(); return { plan, analysis, logoAnalysis }; }
...
async function smartFitBackground(options = {}) { if (smartBusy) return; const fullReset = !!options.fullReset; const btn=options.button || $("smartFit"); smartBusy=true; if(btn){btn.disabled=true;btn.dataset.label=btn.textContent;} const steps=["正在分析 Logo 配色","正在分析背景材质","正在比较深浅文字","正在调整卡片布局"]; let step=0; const advance=()=>{ if(!btn)return; btn.textContent=steps[Math.min(step++,steps.length-1)]; if($("smartSummary"))$("smartSummary").textContent=btn.textContent;}; try { beginInteraction(); advance(); if (state.logo && state.logoCropMode === "auto") await refreshLogoCropBounds(); const analysis = await analyzeBackground(); advance(); const chosen = chooseSmartMaterial(analysis); suppressSmartManual = true; applyMaterial(chosen.key, "smart"); state.glassSaturation = chosen.glass.saturation; state.glassContrast = chosen.glass.contrast; state.glassTintColor = chosen.glass.tintColor; state.glassTintOpacity = chosen.glass.tintOpacity; const material = { ...MATERIAL_PRESETS[chosen.key], saturation:state.glassSaturation, contrast:state.glassContrast, tintColor:state.glassTintColor, tintOpacity:state.glassTintOpacity }; const logoAnalysis = await analyzeLogoColors(chosen.key, analysis.text); state.primaryAccent = logoAnalysis.usable ? logoAnalysis.primaryAccent : safeAccentFromBackground(analysis, material); state.secondaryAccent = logoAnalysis.usable ? logoAnalysis.secondaryAccent : ""; state.accentColor = state.primaryAccent || material.recommendedAccentColor || "#9AA8BB"; advance(); const text = await applySmartTextColors({ resetOpacity: fullReset }); advance(); applySmartLayoutForContent(); state.smartConfidence = chosen.confidence; const familyLabel = text.plan.summaryFamily === "dark" ? "深灰紫标题" : "柔和浅色标题"; const bodyLabel = text.plan.summaryFamily === "dark" ? "暖灰正文" : "浅色正文"; const accentLabel = logoAnalysis.usable ? `${describeAccent(state.accentColor)}强调色` : "安全强调色"; const cardLabel = state.layoutPreset === "system" && state.cardHeightMode === "auto" && state.cardHeight < 620 ? "紧凑卡片" : "自适应卡片"; state.smartSummary = `已应用：${MATERIAL_PRESETS[chosen.key].label} · ${familyLabel} · ${accentLabel} · ${cardLabel}`; const reason = `${chosen.reason}；标题区域明度${regionLumStats(text.analysis.title || text.analysis.text).median > .5 ? "较高" : "较低"}，已同时比较浅色与深色候选，按局部最差对比度选择${familyLabel}。`; showToast(reason); suppressSmartManual = false; syncUi(); scheduleRender(); endInteraction(); } catch (_) { suppressSmartManual = true; applyMaterial("standard", "smart"); state.glassSaturation=.96; state.glassContrast=1.03; state.glassTintColor="#FFFFFF"; state.glassTintOpacity=.12; state.titleColor="#514C52"; state.titleColorMode="auto"; state.bodyTextColor="#746A70"; state.bodyTextColorMode="auto"; state.subtitleColor="#B66F90"; state.subtitleColorMode="auto"; if (fullReset || !state.titleOpacityManuallyEdited) state.titleOpacity=96; if (fullReset || !state.bodyTextOpacityManuallyEdited) state.bodyTextOpacity=84; if (fullReset || !state.subtitleOpacityManuallyEdited) state.subtitleOpacity=88; state.titleColorSource="安全深色回退"; state.bodyTextColorSource="安全暖灰回退"; state.subtitleColorSource="安全粉色回退"; state.titleContrast=3.5; state.bodyTextContrast=4.5; state.subtitleContrast=3.0; state.titleShadowOpacity=.02; state.descShadowOpacity=.02; syncLegacyTextColor(); state.smartConfidence="low"; applySmartLayoutForContent(); state.smartSummary="特征较弱，已应用深色文字安全方案。"; suppressSmartManual = false; syncUi(); scheduleRender(); endInteraction(); showToast("智能适配遇到异常，已应用深色文字安全方案。", "error", true); } finally { smartBusy=false; if(btn){btn.disabled=false;btn.textContent=btn.dataset.label||"智能适配底图";} } }
...
function drawCompatibleTypography(ctx, card, logoBox, typography) {
  ...
  if (state.layoutPreset === "certificate") {
    ...
    if (showDivider) {
      ...
      grad.addColorStop(0, hexToRgba(state.accentColor, .28 * clamp(state.dividerOpacity,0,100) / 100));
