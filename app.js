...
function textMetrics(size, weight, spacing = 0) {
  const canvas = document.createElement("canvas");
  const ctx = canvas.getContext("2d");
  ctx.font = `${weight} ${size}px ${fontStack()}`;
  const metrics = ctx.measureText("成就 Achievement 0123");
  return { ascent: metrics.actualBoundingBoxAscent || size * .78, font: ctx.font, spacing };
}
function descriptionLines(maxLines) {
  const lines = [];
...
function renderCard(card, logoBox) {
  ...
  const descMetrics = textMetrics(descFontSize, 500, state.descLetterSpacing);
  ...
  const lineEnd = contentX + lineWidth;
  return `${glass}${logoLayer}<g class="font-main text-fill"><text x="${contentX}" y="${subtitleY}" class="subtitle">${escapeXml(state.subTitle)}</text><text x="${contentX}" y="${titleY}" class="title" font-size="${title.size}">${escapeXml(state.mainTitle)}</text><line x1="${contentX}" y1="${lineY}" x2="${lineEnd}" y2="${lineY}" stroke="#fff" stroke-opacity=".12" stroke-width="1.25"/><text x="${contentX}" y="${descBaselineY}" class="desc">${desc.lines.map((line,i) => `<tspan x="${contentX}" dy="${i ? state.descLineGap : 0}">${escapeXml(line)}</tspan>`).join("")}</text></g>`;
}
