...
const DEFAULTS = {
  ...
  backgroundScale: 120, backgroundX: 0, backgroundY: 0, backgroundOpacity: 100,
...
const RANGE_CONFIG = {
  backgroundScale: [120, 240, 1], backgroundX: [-100, 100, 1], backgroundY: [-100, 100, 1], backgroundOpacity: [0, 100, 1],
...
function getImagePlacement(area, scale, offsetX, offsetY, expand = 1) { const baseWidth = area.width * (scale / 100), baseHeight = area.height * (scale / 100); const bgAspect = state.backgroundAspect || (area.width / area.height); const areaAspect = area.width / area.height; let renderWidth, renderHeight; if (bgAspect > areaAspect) { renderHeight = baseHeight; renderWidth = baseHeight * bgAspect; } else { renderWidth = baseWidth; renderHeight = baseWidth / bgAspect; } const maxX = Math.max(0, (renderWidth - area.width) / 2), maxY = Math.max(0, (renderHeight - area.height) / 2); const width = renderWidth * expand, height = renderHeight * expand; return { x: area.x + (area.width - renderWidth) / 2 + offsetX / 100 * maxX - (width - renderWidth) / 2, y: area.y + (area.height - renderHeight) / 2 + offsetY / 100 * maxY - (height - renderHeight) / 2, width, height }; }
